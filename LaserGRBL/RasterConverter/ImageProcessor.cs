    ...
    public class ImageProcessor : ICloneable
    {
        ...
        public ImageProcessor(GrblCore core, string fileName, Size boxSize, bool append)
        {
            ...
            using (Bitmap loadedBmp = new Bitmap(fileName))
            {
                ...
                using (Bitmap tmpBmp = new Bitmap(loadedBmp))
                    mOriginal = tmpBmp.Clone(new Rectangle(0, 0, tmpBmp.Width, tmpBmp.Height),
        ...
        public void AutoTrim()
        {
            ...
            if (!bgcolor.IsEmpty)
            {
                ...
                Rectangle scaled = new Rectangle(trim[2], trim[0], mOriginal.Width - trim[2] - trim[3],
                    mOriginal.Height - trim[0] - trim[1]);
        ...
        private static Color ColorAvg(Color c1, Color c2)
        {
            return Color.FromArgb((c1.A + c2.A) / 2, (c1.R + c2.R) / 2, (c1.G + c2.G) / 2, (c1.B + c2.B) / 2);
        }

        private static bool IsSimilarColor(Color c1, Color c2, int tolerance = 20)
        {
        ...
        private void AbortThread()
        {
            if ((TH != null) && TH.ThreadState != System.Threading.ThreadState.Stopped)
            {
                ...
                if (!object.ReferenceEquals(System.Threading.Thread.CurrentThread, TH))
                {
                    ...
                    if (TH != null && TH.ThreadState != System.Threading.ThreadState.Stopped)
                    {
                        System.Diagnostics.Debug.WriteLine(
                            string.Format("Devo forzare la terminazione del Thread '{0}'", TH.Name));
                        TH.Abort();
                    }
                }
                else
                {
                    System.Diagnostics.Debug.WriteLine(
                        string.Format("ATTENZIONE! Chiamata rientrante a thread stop '{0}'", TH.Name));
                }
            }
        ...
        */
        ...
        private void PreviewCenterline(Bitmap bmp)
        {
            try
            {
                ...
                Svg.SvgDocument svg = Autotrace.BitmapToSvgDocument(bmp, UseCornerThreshold, CornerThreshold,
                    UseLineThreshold, LineThreshold);
            ...
            catch (Exception ex)
            {
                using (Graphics g = Graphics.FromImage(bmp))
                {
                    ...
                    g.DrawString(ex.Message, SystemFonts.DefaultFont, Brushes.Red,
        ...
        void DoTrueWork()
        {
            ...
            {
                int maxSize =
                    Tools.OSHelper.Is64BitProcess
                        ? 22000 * 22000
                        : 6000 * 7000; //on 32bit OS we have memory limit - allow Higher value on 64bit
                ...
                if (res > 0)
                {
                    using (Bitmap bmp = CreateTarget(pixelSize))
                    {
                        ...
                        if (SelectedTool == Tool.Line2Line || SelectedTool == Tool.Dithering ||
                            SelectedTool == Tool.NoProcessing)
                            mCore.LoadedFile.LoadImageL2L(bmp, mFileName, conf, mAppend);
                        else if (SelectedTool == Tool.Vectorize)
                            mCore.LoadedFile.LoadImagePotrace(bmp, mFileName, UseSpotRemoval, (int) SpotRemoval,
                                UseSmoothing, Smoothing, UseOptimize, Optimize, OptimizeFast, conf, mAppend, mCore);
                        else if (SelectedTool == Tool.Centerline)
                            mCore.LoadedFile.LoadImageCenterline(bmp, mFileName, UseCornerThreshold, CornerThreshold,
        ...
        private Bitmap ProduceBitmap(Image img, Size size)
        {
            ...
            {
                using (Image downsampled = ImageTransform.ResizeImage(img,
                    new Size((int) (size.Width * 1 / DownSampling), (int) (size.Height * 1 / DownSampling)), false,
        ...
        private Bitmap ProduceWhitepointDemo(Image img, Size size)
        {
            ...
            using (Bitmap grayscale = ImageTransform.GrayScale(resized, Red / 100.0F, Green / 100.0F, Blue / 100.0F,
                -((100 - Brightness) / 100.0F), (Contrast / 100.0F),
                IsGrayScale ? ImageTransform.Formula.SimpleAverage : Formula))
        ...
        private Bitmap ProduceBitmap2(Image img, ref Size size)
        {
            ...
            else
            {
                ...
                {
                    using (Bitmap grayscale = ImageTransform.GrayScale(resized, Red / 100.0F, Green / 100.0F,
                        Blue / 100.0F, -((100 - Brightness) / 100.0F), (Contrast / 100.0F),
                        IsGrayScale ? ImageTransform.Formula.SimpleAverage : Formula))
                    {
                        using (Bitmap whiten = ImageTransform.Whitenize(grayscale, mWhitePoint, false))
                        {
                            ...
                            else if (SelectedTool == Tool.Centerline)
                            {
                                ...
                                return ImageTransform.Threshold(
                                    ImageTransform.Threshold(whiten, Threshold / 100.0F, UseThreshold), 50.0F / 100.0F,
                                    true);
                            }
                            else
        ...
        private void PreviewLineByLine(Bitmap bmp)
        {
            ...
            if (!MustExitTH && dir != Direction.None)
            {
                using (Graphics g = Graphics.FromImage(bmp))
                {
                    ...
                    if (dir == Direction.Horizontal || dir == Direction.NewHorizontal || dir == Direction.NewGrid ||
                        dir == Direction.NewCross)
                    {
                    ...
                    if (dir == Direction.Vertical || dir == Direction.NewVertical || dir == Direction.NewGrid ||
                        dir == Direction.NewCross)
                    {
                    ...
                    if (dir == Direction.Diagonal || dir == Direction.NewDiagonal || dir == Direction.NewDiagonalGrid ||
                        dir == Direction.NewDiagonalCross || dir == Direction.NewSquares || dir == Direction.NewZigZag)
                    {
                    ...
                    if (dir == Direction.NewReverseDiagonal || dir == Direction.NewDiagonalGrid ||
                        dir == Direction.NewDiagonalCross || dir == Direction.NewSquares)
                    {
                        int alpha = SelectedTool == ImageProcessor.Tool.Dithering ? 150 : 255;
                        for (int I = 0; I < bmp.Width + bmp.Height - 1 && !MustExitTH; I++)
                        {
        ...
        private void PreviewVector(Bitmap bmp)
        {
            ...
            using (Graphics g = Graphics.FromImage(bmp))
            {
                ...
                using (Brush fill =
                    new SolidBrush(Color.FromArgb(FillingDirection != Direction.None ? 255 : 30, Color.Black)))
        ...
        public Bitmap TrueOriginal
        {
            get { return mOriginal; }
        } //originale eventualmente croppata e ruotata
